...
var transform = require("css-to-react-native"), React = require("react"), react = require("@emotion/react");
...
var styles, transform__default = _interopDefault(transform);
  ...
  if ("" !== str.trim()) {
    ...
    try {
      ...
      if (msg.includes("Failed to parse declaration")) {
        var values = msg.replace("Failed to parse declaration ", "").replace(/"/g, "").trim().split(":"), errorMsg = "'" + values[0] + "' shorthand property requires units for example - " + values[0] + ": 20px or " + values[0] + ": 10px 20px 40px 50px";
...
function createStyled(StyleSheet, _temp) {
  ...
  return function createEmotion(component, options) {
    var shouldForwardProp = options && options.shouldForwardProp ? options.shouldForwardProp : void 0, defaultShouldForwardProp = shouldForwardProp || getShouldForwardProp(component), shouldUseAs = !defaultShouldForwardProp("as");
    return function() {
      ...
      var Styled = React.forwardRef((function(props, ref) {
        var finalTag = shouldUseAs && props.as || component, mergedProps = props;
        if (null == props.theme) {
          for (var key in mergedProps = {}, props) mergedProps[key] = props[key];
          mergedProps.theme = React.useContext(react.ThemeContext);
        }
        var finalShouldForwardProp = shouldUseAs && void 0 === shouldForwardProp ? getShouldForwardProp(finalTag) : defaultShouldForwardProp, newProps = {};
        for (var _key2 in props) shouldUseAs && "as" === _key2 || finalShouldForwardProp(_key2) && (newProps[_key2] = props[_key2]);
        return newProps.style = [ css.apply(mergedProps, styles), props.style ], newProps.ref = ref, 
